...
// Yi
        realtype S0;
        realtype SbarUst;       /*Averaged Soil Organic Carbon in unsaturated zone Unit: mg/g*/
        realtype SbarSat;       /*Averaged Soil Organic Carbon in saturated zone Unit: mg/g*/
        realtype SMasUst;        /*  Soil Organic Carbon Mass in saturated zone Unit: mg*/
        realtype SMasSat;        /* Soil Organic Carbon in saturated zone Unit: mg*/
//Yi
	} element;
...
typedef struct model_data_structure 	/* Model_data definition */
  ...
  /* Transport Parameters START */
  TRANSPORT int NumSolute, SoluteInitMode, AgeInitMode, AGE;
  TRANSPORT realtype **EleConc, **EleAgeConc, *CZero;
  TRANSPORT realtype **RivConc, **RivAgeConc, *AZero;  
  TRANSPORT int AlphaInitModeSurf, AlphaInitModeRiv, AlphaInitModeSat;
  TRANSPORT realtype DiffusionSurf, DiffusionRiv, DiffusionSat;
  TRANSPORT realtype AlphaLSurf, AlphaLRiv, AlphaLSat;
  TRANSPORT realtype AlphaTSurfFrac, AlphaTRivFrac, AlphaTSatFrac; 
  TRANSPORT TSD *TSD_PChem;
  //Yi
  TRANSPORT realtype *EleS,*EleSbar;
  TRANSPORT realtype Tau, Kd,somAlpha,CalS0, Bd,kovld;
  TRANSPORT realtype *gwtb_old;
  TRANSPORT realtype *unsatY_old;
  //Yi
  /* Transport Parameters END */
...
//  	TSD *TSD_DH;		     	/* Zero plane Displacement Height */  
	...
	realtype *EleISsnowmax;		/* Maximum interception storage (snow) */
  	...
  	realtype Q; 
 	realtype *DummyY; 
	realtype *PrintVar[33];
	processCal pcCal;
realtype inigw, inius,iniov,iniriv,inibriv,tgw, tus, rsflx, et0flx,et1flx,et2flx, pflx, tsnowc, tsnowg,tis,tov ,tbriv,triv,Tarea;  //Yi variable for mass balance check
...
realtype returnVal(realtype,realtype,realtype,realtype);//Yi
realtype CS_AreaOrPerem(int,realtype,realtype,realtype);//Yi
