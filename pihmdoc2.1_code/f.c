...
//            MD->Ele[i].gwdpt = MD->Ele[i].zmax - MD->Ele[i].zmin - Y[2*MD->NumEle + i];
//            printf("%lf\t",MD->Ele[i].gwdpt);
//        }
//            getchar();

	/* Initialization of temporary state variables */
	for(i=0; i<(3*MD->NumEle + 2*MD->NumRiv) * (1+MD->NumSolute*(1+MD->AGE)); i++)
		DY[i]=0.0;
