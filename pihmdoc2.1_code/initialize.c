...
//        FILE *cdnt;
        FILE *inidoc;
        realtype AquiferDepth,UsatDepth;
        int jj;
//        cdnt = fopen("xycrdnt.txt","w");
// Yi
	 ...
	 for(i=0;i<DS->NumEle;i++)
		{
    		...
    		DS->Ele[i].area = 0.5*((b_x - a_x)*(c_y - a_y) - (b_y - a_y)*(c_x - a_x));
                DS->Tarea = DS->Tarea + DS->Ele[i].area; //Yi
...
//                fprintf(cdnt,"%d\t%lf\t%lf\n",i+1,DS->Ele[i].x,DS->Ele[i].y);
	...
	//	DS->Ele[i].Porosity = (DS->Soil[(DS->Ele[i].soil-1)].ThetaS - DS->Soil[(DS->Ele[i].soil-1)].ThetaR); //?? XUAN FOR SHALEHILLS ONLY
...
//    		DS->Ele[i].Beta = CS->Cal.Beta*DS->Geol[(DS->Ele[i].geol-1)].Beta; 
  	...
  	for(i=0; i<DS->NumRiv; i++)
		...
		DS->Riv[i].BedSlope = (DS->Node[DS->Riv[i].FromNode-1].zmax - DS->Node[DS->Riv[i].ToNode-1].zmax) / (DS->Riv[i].Length);
...
//		DS->Ele[i+DS->NumEle].zmin=DS->Riv[i].zmax-40;
	...
	/* Memory allocation of print variables */	
        for(i=0;i<33;i++)
                ...
                else if((i>=7)&&(i<19) || (i==26) || (i==27) || (i==31) || (i==32))
                        {
...
ini_som = fopen("inisom.txt","w");
    		if(init_file == NULL)
				...
				NV_Ith_S(CV_Y, i + 2*DS->NumEle)=tempvalue3;	
                               //Yi
 //for water balance
                                DS->iniov = DS->iniov + DS->Ele[i].area*tempvalue1/DS->Tarea; //Yi
                                DS->inius = DS->inius + DS->Ele[i].Porosity*DS->Ele[i].area*tempvalue2/DS->Tarea; //Yi
                                DS->inigw = DS->inigw + DS->Ele[i].Porosity*DS->Ele[i].area*tempvalue3/DS->Tarea; //Yi
//for water balance
                                AquiferDepth = DS->Ele[i].zmax - DS->Ele[i].zmin; 
                                if (tempvalue3 > 0){
                                UsatDepth = (AquiferDepth - tempvalue3)>tempvalue2? (AquiferDepth - tempvalue3):tempvalue2;
                                }else{
                                UsatDepth = AquiferDepth;
                                }
                                DS->Ele[i].S0 = DS->Soil[DS->Ele[i].soil - 1].som * AquiferDepth * DS->CalS0;//get the top som
                                if (tttt>DS->Ele[i].S0){
                                    tttt = tttt;
                                }else
                                {
                                ...
                                fprintf(ini_som,"%lf\n",tttt);
                                if (tempvalue3 > 0){
                                DS->EleSbar[i + 0*DS->NumEle] = DS->Ele[i].S0*(1-exp(-DS->somAlpha*UsatDepth)) / (UsatDepth*DS->somAlpha);
                                DS->EleSbar[i + 1*DS->NumEle] = DS->Ele[i].S0*(exp(-DS->somAlpha*UsatDepth)-exp(-DS->somAlpha*AquiferDepth)) / (tempvalue3*DS->somAlpha);
				DS->EleS[i + 0*DS->NumEle] = DS->EleSbar[i + 0*DS->NumEle] * UsatDepth * DS->Ele[i].area * DS->Bd;
                                DS->EleS[i + 1*DS->NumEle] = DS->EleSbar[i + 1*DS->NumEle] * tempvalue3 * DS->Ele[i].area * DS->Bd;
                                } else {
                                DS->EleSbar[i + 0*DS->NumEle] = DS->Ele[i].S0*(1-exp(-DS->somAlpha*AquiferDepth)) / (AquiferDepth*DS->somAlpha);
                                DS->EleSbar[i + 1*DS->NumEle] = 0.0;
				DS->EleS[i + 0*DS->NumEle] = DS->EleSbar[i + 0*DS->NumEle] * UsatDepth * DS->Ele[i].area * DS->Bd;
                                DS->EleS[i + 1*DS->NumEle] = 0.0;
                                }
                               //Yi
      				}  
      			for(i=0; i<DS->NumRiv; i++)
...
// Yi
    DS->gwtb_old = (realtype *)malloc(DS->NumEle*sizeof(realtype));  
    DS->unsatY_old = (realtype *)malloc(DS->NumEle*sizeof(realtype));  
    for (i=0; i<DS->NumEle;i++){
	DS->gwtb_old[i] = NV_Ith_S(CV_Y,i + 2*DS->NumEle); 
	DS->unsatY_old[i] = NV_Ith_S(CV_Y,i + 1*DS->NumEle); 
        }
//    printf("%lf %lf\n",DS->gwtb_old[1],DS->gwtb_old[10]);   

          for(i=0; i<DS->NumSolute; i++){
                  for(j=0; j<DS->NumEle; j++){
                          DS->EleConc[j+1*DS->NumEle][i]=DS->EleSbar[j + 0*DS->NumEle]/DS->Kd;
                          DS->EleConc[j+0*DS->NumEle][i]=DS->EleConc[j+1*DS->NumEle][i];
                          DS->EleConc[j+2*DS->NumEle][i]=DS->EleSbar[j + 1*DS->NumEle]/DS->Kd;
                     //     if (DS->EleSbar[j + 1*DS->NumEle]<0.0001){
                     //     DS->EleSbar[j + 1*DS->NumEle]=0.0001;
                     //     DS->EleConc[j+2*DS->NumEle][i]=DS->EleSbar[j + 1*DS->NumEle]/DS->Kd;
                     //     }
                  }
          }
//           printf("%lf\t%lf\t%lf\t",DS->EleConc[235+1*DS->NumEle][0],DS->EleSbar[235 + 0*DS->NumEle],DS->Kd);
// getchar();
          for(i=0; i<DS->NumSolute; i++){
                  for(j=0; j<DS->NumRiv; j++){
                          DS->RivConc[j+0*DS->NumRiv][i]=(DS->EleConc[2*DS->NumEle + DS->Riv[j].LeftEle-1][i]+DS->EleConc[2*DS->NumEle + DS->Riv[j].RightEle-1][i])/2.0;
                          DS->RivConc[j+1*DS->NumRiv][i]=(DS->EleConc[2*DS->NumEle + DS->Riv[j].LeftEle-1][i]+DS->EleConc[2*DS->NumEle + DS->Riv[j].RightEle-1][i])/2.0;
                  }
          }
         inidoc = fopen("wccdoc.init","w");
         for(jj=0; jj<DS->NumEle; jj++){
                  fprintf(inidoc, "%lf \t %.10lf \t %.20lf \t %lf \t %lf \t %.20lf\n", DS->EleConc[jj+0*DS->NumEle][0],DS->EleConc[jj+1*DS->NumEle][0],DS->EleConc[jj+2*DS->NumEle][0],DS->Ele[jj].S0,DS->EleSbar[jj + 0*DS->NumEle], DS->EleSbar[jj + 1*DS->NumEle]);
         }
         for(jj=0; jj<DS->NumRiv; jj++){
                  fprintf(inidoc, "%lf \t %lf\n", DS->RivConc[jj+0*DS->NumRiv][0], DS->RivConc[jj+1*DS->NumRiv][0]);
         }
         fclose(inidoc);
// Yi
//TRANSPORT
    for(i=0; i<DS->NumSolute; i++){
	for(j=0; j<3*DS->NumEle; j++){
		//??if(i==0 && j>=2*DS->NumEle) DS->EleConc[j][i]=8.0; else DS->EleConc[j][i]=0.0;
		NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv) + i*3*DS->NumEle + j) = DS->EleConc[j][i];
		//?? mass NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv) + i*3*DS->NumEle + j) = DS->EleConc[j][i]*NV_Ith_S(CV_Y,j)*DS->Ele[j%DS->NumEle].area;
		//if(j==2*DS->NumEle+9) {printf("\n->%lf\n", DS->EleConc[j][i]); getchar();}
		if(DS->AGE==1){
			NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv)*(DS->NumSolute+1) + i*3*DS->NumEle + j) = DS->EleAgeConc[j][i];
		}
	}
    }
	//NV_Ith_S(CV_Y, (3*DS->NumEle + DS->NumRiv) + 2*DS->NumEle+666 + 0) = 1.0;  //??

	//NV_Ith_S(CV_Y, (3*DS->NumEle + DS->NumRiv) + 666) = 2.0;  //??

    for(i=0; i<DS->NumSolute; i++){
	for(j=0; j<2*DS->NumRiv; j++){
		//??DS->RivConc[j][i]=8.0;
		NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv) + DS->NumSolute*3*DS->NumEle+j + i*2*DS->NumRiv) = DS->RivConc[j][i];
		//?? mass NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv) + DS->NumSolute*3*DS->NumEle+j + i*2*DS->NumRiv) = DS->RivConc[j][i]*NV_Ith_S(CV_Y,3*DS->NumEle+j)*DS->Riv[j%DS->NumRiv].coeff;
		if(DS->AGE==1){
			NV_Ith_S(CV_Y, (3*DS->NumEle + 2*DS->NumRiv)*(DS->NumSolute+1) + DS->NumSolute*3*DS->NumEle + i*2*DS->NumRiv + j) = DS->RivAgeConc[j][i];
		}
	}
    }
	//NV_Ith_S(CV_Y, (3*DS->NumEle + DS->NumRiv) + DS->NumSolute*3*DS->NumEle+0 + 0*DS->NumRiv) = 3.0;

    printf("done.\n");
	DS->CZero = (realtype *)malloc(DS->NumSolute*(3*DS->NumEle+DS->NumRiv)*sizeof(realtype));
	DS->AZero = (realtype *)malloc(DS->NumSolute*(3*DS->NumEle+DS->NumRiv)*sizeof(realtype));
	for(i=0; i<DS->NumSolute; i++){
		for(j=0; j<3*DS->NumEle; j++){
			DS->CZero[i*3*DS->NumEle+j]=0.0;
			DS->AZero[i*3*DS->NumEle+j]=0.0;
		}
		for(k=0; k<  DS->NumRiv; k++){
			DS->CZero[DS->NumSolute*3*DS->NumEle + i*DS->NumRiv + k] = 0.0;
			DS->AZero[DS->NumSolute*3*DS->NumEle + i*DS->NumRiv + k] = 0.0;
		}
	}
