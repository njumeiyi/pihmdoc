...
TRANSPORT void read_alloc_solute(char *filename, Model_Data DS, Control_Data *CS);
...
void read_alloc(char *filename, Model_Data DS, Control_Data *CS)
	...
	FILE *global_calib;	/* Pointer to .calib file */
  	...
  	for (i=0; i<DS->NumSoil; i++)
		...
		/* Note: Soil KsatH and macKsatH is not used in model calculation anywhere */
    		...
    		fscanf(soil_file, "%lf", &(DS->Soil[i].som));
	...
	TRANSPORT read_alloc_solute(filename, DS, CS);
}


TRANSPORT
void read_alloc_solute(char *filename, Model_Data DS, Control_Data *CS){

  TRANSPORT FILE *solute_file; FILE *pchem_file;
  // Yi
  TRANSPORT FILE *som_file;
  // Yi
  //
  char *fn[12];
  int i, j, k, tempindex;
  ...
  realtype alpha;
  /*========== open *.solute file ==========*/
  printf("\n  11) reading %s.solute ... ", filename);
  fn[8] = (char *)malloc((strlen(filename)+8)*sizeof(char));
  strcpy(fn[8], filename);
  solute_file = fopen(strcat(fn[8], ".solute"), "r");

  if(solute_file == NULL)
  {
    printf("\n  Fatal Error: %s.solute is in use or does not exist!\n", filename);
    exit(1);
  }

  /* start reading solute_file */  
  //read NumSolute
  fscanf(solute_file, "%d %d", &DS->NumSolute, &DS->AGE);
  //DS->NumSolute = 1;
if(DS->NumSolute>0){
  fscanf(solute_file, "%d %d %d", &DS->AlphaInitModeSurf, &DS->AlphaInitModeRiv, &DS->AlphaInitModeSat);
  				// 0->Flat Values, 1,2,3...->Using Imperical Equation Number
  fscanf(solute_file, "%lf %lf %lf", &DS->AlphaLSurf, &DS->AlphaLRiv, &DS->AlphaLSat);
  fscanf(solute_file, "%lf %lf %lf", &DS->AlphaTSurfFrac, &DS->AlphaTRivFrac, &DS->AlphaTSatFrac);
  fscanf(solute_file, "%lf %lf %lf", &DS->DiffusionSurf, &DS->DiffusionRiv, &DS->DiffusionSat);
// Yi
  fscanf(solute_file, "%lf %lf %lf %lf %lf %lf", &DS->Tau, &DS->Kd, &DS->somAlpha, &DS->CalS0, &DS->Bd, &DS->kovld);
  DS->EleS = (realtype *)malloc(2*DS->NumEle*sizeof(realtype *));
  DS->EleSbar = (realtype *)malloc(2*DS->NumEle*sizeof(realtype *));
 // Yi
  fscanf(solute_file, "%d", &DS->SoluteInitMode);
  DS->EleConc = (realtype **)malloc(3*DS->NumEle*sizeof(realtype *));
  DS->RivConc = (realtype **)malloc(2*DS->NumRiv*sizeof(realtype *));

  if(DS->AGE == 1){
	  DS->EleAgeConc = (realtype **)malloc(3*DS->NumEle*sizeof(realtype *));
	  DS->RivAgeConc = (realtype **)malloc(2*DS->NumRiv*sizeof(realtype *));
  }
  for(i=0; i<3*DS->NumEle; i++){
	  DS->EleConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
	  if(DS->AGE==1)
		  DS->EleAgeConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
  }
  for(i=0; i<2*DS->NumRiv; i++){
	  DS->RivConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
	  if(DS->AGE==1)
		  DS->RivAgeConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
  }
  if(DS->SoluteInitMode == 1){ //Uniform solute init condition
	  for(i=0; i<DS->NumSolute; i++){
		  fscanf(solute_file, "%s %d",tempchar, &tempindex);
		  fscanf(solute_file, "%lf %lf %lf", &tempValue1, &tempValue2, &tempValue3);
		  for(j=0; j<DS->NumEle; j++){
			  DS->EleConc[j+0*DS->NumEle][i]=tempValue1;
			  DS->EleConc[j+1*DS->NumEle][i]=tempValue2;
			  DS->EleConc[j+2*DS->NumEle][i]=tempValue3;
		  }
	  }
	  for(i=0; i<DS->NumSolute; i++){
		  fscanf(solute_file, "%s %d", tempchar, &tempindex);
		  fscanf(solute_file, "%lf %lf", &tempValue1, &tempValue2);
		  for(j=0; j<DS->NumRiv; j++){
			  DS->RivConc[j+0*DS->NumRiv][i]=tempValue1;
			  DS->RivConc[j+1*DS->NumRiv][i]=tempValue2;
		  }
	  }
  }
 /* else if(DS->SoluteInitMode == 2){ //Distributed solute init condition TODO
         // Yi
          for(i = 0; i < DS->NumEle; i++){
              DS->Ele[i].som0 = DS->Soil[DS->Ele[i].soil - 1].som*(DS->Ele[i].zmax - DS->Ele[i].zmin)*DS->alphasom;//get the s value on the top of soil
          }
	  for(i=0; i<DS->NumSolute; i++){
		  fscanf(solute_file, "%s %d", tempchar, &tempindex);
		  for(j=0; j<DS->NumEle; j++){
			  fscanf(solute_file, "%lf %lf %lf", &tempValue1, &tempValue2, &tempValue3);
			  DS->EleConc[j+0*DS->NumEle][i]=DS->Ele[j].som/DS->Kd;
			  DS->EleConc[j+1*DS->NumEle][i]=DS->Ele[j].som/DS->Kd;
	                  DS->EleConc[j+2*DS->NumEle][i]=DS->Ele[j].som/DS->Kd;
		  }
	  }
	  for(i=0; i<DS->NumSolute; i++){
		  fscanf(solute_file, "%s %d", tempchar, &tempindex);
		  for(j=0; j<DS->NumRiv; j++){
			  fscanf(solute_file, "%lf", &tempValue1);
			  DS->RivConc[j+0*DS->NumRiv][i]=tempValue1;
			  DS->RivConc[j+1*DS->NumRiv][i]=tempValue2;
		  }
	  }
	  ...
	  printf("Fatal Error: Unknown Solute INIT Type - CORRECT .solute file :: HALTING...\n");
	  getchar(); getchar();
  }*//*
  for(i=0; i<3*DS->NumEle; i++){
	DS->EleConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
	for(j=0; j<DS->NumSolute; j++){
		///read solute concentration
		if(j==0) DS->EleConc[i][j] = 1.0; else DS->EleConc[i][j] = 0.0;
	}
  }

  DS->RivConc = (realtype **)malloc(DS->NumRiv*sizeof(realtype *));
  for(i=0; i<DS->NumRiv; i++){
	DS->RivConc[i] = (realtype *)malloc(DS->NumSolute*sizeof(realtype));
	for(j=0; j<DS->NumSolute; j++){
		///read solute concentration
  ...
  if(DS->AGE==1){
	  fscanf(solute_file, "%d", &DS->AgeInitMode);
	  if(DS->AgeInitMode == 1){ //Uniform Age init condition
		  for(i=0; i<DS->NumSolute; i++){
			  fscanf(solute_file, "%s %d",tempchar, &tempindex);
			  fscanf(solute_file, "%lf %lf %lf", &tempValue1, &tempValue2, &tempValue3);
			  for(j=0; j<DS->NumEle; j++){
				  DS->EleAgeConc[j+0*DS->NumEle][i]=tempValue1*DS->EleConc[j+0*DS->NumEle][i];
				  DS->EleAgeConc[j+1*DS->NumEle][i]=tempValue2*DS->EleConc[j+1*DS->NumEle][i];
				  DS->EleAgeConc[j+2*DS->NumEle][i]=tempValue3*DS->EleConc[j+2*DS->NumEle][i];
			  }
		  }
		  for(i=0; i<DS->NumSolute; i++){
			  fscanf(solute_file, "%s %d", tempchar, &tempindex);
			  fscanf(solute_file, "%lf %lf", &tempValue1, &tempValue2);
			  for(j=0; j<DS->NumRiv; j++){
				  DS->RivAgeConc[j+0*DS->NumRiv][i]=tempValue1*DS->RivConc[j+0*DS->NumRiv][i];
                                  DS->RivAgeConc[j+1*DS->NumRiv][i]=tempValue2*DS->RivConc[j+1*DS->NumRiv][i];
			  }
		  }
	  }
	  else if(DS->AgeInitMode == 2){ //Distributed Age Init Condition TODO
		  for(i=0; i<DS->NumSolute; i++){
			  fscanf(solute_file, "%s %d", tempchar, &tempindex);
			  for(j=0; j<DS->NumEle; j++){
				  fscanf(solute_file, "%lf %lf %lf", &tempValue1, &tempValue2, &tempValue3);
		  ...
		  for(i=0; i<DS->NumSolute; i++){
			  ...
			  for(j=0; j<DS->NumRiv; j++){
				  fscanf(solute_file, "%lf", &tempValue1);
				  DS->RivAgeConc[j][i]=tempValue1*DS->RivConc[j][i];
			  }
		  }
	  }
	  else{
...
printf("done.\n");
  ...
  /*========== open *.precip chem file ==========*/
  printf("\n  12) reading %s.pchem ... ", filename);
  fn[9] = (char *)malloc((strlen(filename)+6)*sizeof(char));
  strcpy(fn[9], filename);
  //printf("-%s-\n", filename);
  pchem_file = fopen(strcat(fn[9], ".pchem"), "r");

  if(pchem_file == NULL)
  {
	  printf("\n Fatal Error: %s.pchem is in use or does not exist!\n", filename);
	  exit(1);
  }
  //?? Add number of Precip type loop and Number Solute loop
  DS->TSD_PChem = (TSD *)malloc(DS->NumPrep*DS->NumSolute*sizeof(TSD));
  for(i=0; i<DS->NumPrep; i++){
	  fscanf(pchem_file, "%s %d %d", DS->TSD_PChem[i].name, &DS->TSD_PChem[i].index, &DS->TSD_PChem[i].length);

	  for(k=0; k<DS->NumSolute; k++){
	  	DS->TSD_PChem[i+k*DS->NumPrep].TS = (realtype **)malloc((DS->TSD_PChem[i].length)*sizeof(realtype));
	  }
  	  for(j=0; j<DS->TSD_PChem[i].length; j++)
  	{
		for(k=0; k<DS->NumSolute; k++){
			DS->TSD_PChem[i+k*DS->NumPrep].TS[j] = (realtype *)malloc(2*sizeof(realtype));
		}
  	}
  
  	for(j=0; j<DS->TSD_PChem[i].length; j++)
  	{
		fscanf(pchem_file, "%lf", &tempValue1);
		for(k=0; k<DS->NumSolute; k++){
			DS->TSD_PChem[i+k*DS->NumPrep].TS[j][0] = tempValue1;
			fscanf(pchem_file, "%lf",&DS->TSD_PChem[i+k*DS->NumPrep].TS[j][1]);
		}
  	}
	for(k=0; k<DS->NumSolute; k++)
		DS->TSD_PChem[i+k*DS->NumPrep].iCounter=0;
  }
...
//	  for(j=0; j<DS->TSD_PChem[i].length; j++){
//		  for(k=0; k<DS->NumSolute; k++){
//			  printf("%lf\t", DS->TSD_PChem[i+k*DS->NumPrep].TS[j][1]);
//		  }
//		  if(DS->NumSolute>0) printf("\n");
//	  }
//  }

  printf("\nNumber of Solutes        - %d", DS->NumSolute);
  printf("\nCompute Age[0/NAY 1/YEA] - %d", DS->AGE);
